...
import play.cutscenes.dialogue.BasicDialogueBox;
import backend.audio.Section.SwagSection;
import backend.audio.Song.SwagSong;
import play.cutscenes.video.VideoCutscene;
import play.cutscenes.dialogue.Week6DialogueBox;
...
import play.characters.Character;
import play.characters.Boyfriend;
import play.objects.*;
import play.stage.objects.*;
import ui.story.StoryMenuState;
import ui.freeplay.FreeplayState;
#if (MULTIPLE_VOICES || VOICES_GROUP)
import backend.audio.VoicesGroup;
#end
...
class PlayState extends MusicBeatState
{
  ...
  override public function create()
  {
    ...
    var checkForTextDialoguePath = function(path:String) {
      if (Assets.exists(path))
      {
        ...
        if (CoolUtil.coolTextFile(path).length > 0) dialogue = CoolUtil.coolTextFile(path);
      }
    }
    ...
    if (PreferencesMenu.getPref('downscroll')) strumLine.y = FlxG.height - 150; // 150 just random ass number lol
    ...
    FlxG.camera.follow(camFollow, LOCKON, 0.04 / (FlxG.updateFramerate / 60));
  ...
  function initDiscord():Void
  {
    ...
    Discord.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
    #end
  }
  ...
  function basicDialogue(?dialogueBox:BasicDialogueBox):Void
  {
    ...
    new FlxTimer().start(0.3, function(tmr:FlxTimer) {
      ...
      if (black.alpha > 0) tmr.reset(0.1);
      else
      {
        if (dialogueBox != null)
        {
  ...
  function startSong():Void
  {
    ...
    Discord.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, true, songLength);
    #end
  }
  ...
  private function generateSong():Void
  {
    ...
    #if (MULTIPLE_VOICES || VOICES_GROUP)
    if (SONG.needsVoices) vocals = new VoicesGroup(SONG.song, SONG.vocalsList);
    else
  ...
  override function closeSubState()
  {
    if (paused)
    {
      ...
      #if linc_discord_rpc
      if (startTimer.finished) Discord.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, true,
  ...
  override public function onFocus():Void
  {
    ...
    {
      if (Conductor.songPosition > 0.0) Discord.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, true,
        songLength - Conductor.songPosition);
      else
        Discord.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
    }
  ...
  {
    if (health > 0 && !paused && FlxG.autoPause) Discord.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
  ...
  override public function update(elapsed:Float)
  {
    ...
    scoreTxt.text = "Score: " + songScore.formatMoney(false);
    ...
    if (controls.PAUSE && startedCountdown && canPause)
    {
      ...
      var pauseSubState:PauseSubState = new PauseSubState(boyfriendPos.x, boyfriendPos.y);
      pauseSubState.camera = camHUD;
      openSubState(pauseSubState);
      boyfriendPos.put();

      #if linc_discord_rpc
      Discord.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
      #end
    }

    if (FlxG.keys.justPressed.SEVEN)
    {
      FlxG.switchState(() -> new ui.debug.ChartingState());
    ...
    if (!inCutscene && !_exiting)
    {
      ...
      if (health <= 0 && !practiceMode)
      {
        ...
        Discord.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
        #end
      }
    }
    ...
    if (generatedMusic)
    {
      notes.forEachAlive(function(daNote:Note) {
        ...
        if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByCPU)
        {
          ...
          if (!curStage.contains('school')) enemyStrumNote.offset.subtract(13, 13);
  ...
  function endSong():Void
  {
    ...
    if (isStoryMode)
    {
      ...
      if (storyPlaylist.length <= 0)
      {
        ...
        {
          case 7:
            FlxG.switchState(() -> new ui.title.VideoState());
          default:
  ...
  private function popUpScore(strumtime:Float, daNote:Note):Void
  {
    ...
    if (rating.x < FlxG.camera.scroll.x) rating.x = FlxG.camera.scroll.x;
    else if (rating.x > FlxG.camera.scroll.x + FlxG.camera.width - rating.width) rating.x = FlxG.camera.scroll.x + FlxG.camera.width - rating.width;
  ...
  function displayCombo():Void
  {
    ...
    if (comboSpr.x < FlxG.camera.scroll.x + 194) comboSpr.x = FlxG.camera.scroll.x + 194;
    else if (comboSpr.x > FlxG.camera.scroll.x + FlxG.camera.width - comboSpr.width) comboSpr.x = FlxG.camera.scroll.x + FlxG.camera.width - comboSpr.width;
  ...
  function cameraMovement()
  {
    if (camFollow.x != dad.getMidpoint().x + 150 && !cameraRightSide)
    {
      ...
      if (dad.curCharacter == 'mom') vocals.volume = 1;
    ...
    if (cameraRightSide && camFollow.x != boyfriend.getMidpoint().x - 100)
    {
      ...
      if (SONG.song.toLowerCase() == 'tutorial') FlxTween.tween(FlxG.camera, {zoom: 1}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.elasticInOut});
    }
  }
  ...
  private function keyShit():Void
  {
    ...
    playerStrums.forEach(function(spr:FlxSprite) {
      if (pressArray[spr.ID] && spr.animation.name != 'confirm') spr.animation.play('pressed');
  ...
  override function beatHit()
  {
    ...
    if (curBeat % 2 == 0)
    {
      if (!boyfriend.animation.name.startsWith("sing")) boyfriend.playAnim('idle');
      if (!dad.animation.name.startsWith("sing")) dad.dance();
    }
