...
import backend.graphics.FixedBitmapData;
...
class FunkinCamera extends FlxCamera
{
      ...
      ?shader:FlxShader):Void
  {
    ...
    if (shouldUseShader)
    {
      ...
      _backgroundRenderTexture.init(Std.int(this.width * FlxG.stage.window.scale), Std.int(this.height * FlxG.stage.window.scale));
      _backgroundRenderTexture.drawToCamera((camera, matrix) -> {
        camera.zoom = this.zoom;
        matrix.scale(FlxG.stage.window.scale, FlxG.stage.window.scale);
      ...
      _cameraMatrix.scale(1 / (this.scaleX * FlxG.stage.window.scale), 1 / (this.scaleY * FlxG.stage.window.scale));
