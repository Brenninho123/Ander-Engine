...
import play.objects.Note;
import play.objects.HealthIcon;
import backend.audio.VoicesGroup;
import backend.audio.Song;
...
class ChartingState extends MusicBeatState
{
  ...
  var _song:backend.audio.Song.SwagSong;
  ...
  **/
  ...
  function addVocalUI():Void
  {
    ...
    var addVocal:FlxButton = new FlxButton(UI_vocalAdder.x + UI_vocalAdder.width + 10, UI_vocalAdder.y, 'Add vocal', function() {
      if (!_song.vocalsList.contains(typingShit.text)) _song.vocalsList.push(typingShit.text);
    });
    ...
    var removeVocal:FlxButton = new FlxButton(addVocal.x + addVocal.width + 10, addVocal.y, 'Remove vocal', function() {
      if (_song.vocalsList.contains(typingShit.text)) _song.vocalsList.remove(typingShit.text);
    });
  ...
  function loadSong(daSong:String):Void
  {
    ...
    if (_song.needsVoices) vocals = new VoicesGroup(daSong, _song.vocalsList);
    else
  ...
  override function update(elapsed:Float)
  {
    ...
    if (UI_box.selected_tab_id == 'Vocals') typingShit = UI_vocalAdder;
    ...
    if ((_song?.vocalsList ?? []).length > 0) for (vocal in _song?.vocalsList ?? [])
    {
  ...
  {
    var lastChange:backend.audio.Conductor.BPMChangeEvent =
      {
        stepTime: 0,
        songTime: 0,
        bpm: 0
    ...
    for (i => bpmChange in Conductor.bpmChangeMap)
    {
      if (FlxG.sound.music.time > bpmChange.songTime) lastChange = bpmChange;
    }
  ...
  {
    var sec:backend.audio.Section.SwagSection =
      {
