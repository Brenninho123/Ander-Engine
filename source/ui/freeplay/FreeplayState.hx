...
class FreeplayState extends MusicBeatState
{
  ...
  public inline function addSong(songName:String, weekNum:Int, songCharacter:String)
  {
  ...
  override function update(elapsed:Float)
  {
    ...
    if (accepted)
    {
      ...
      {
        var alertTitle:String = 'Missing Chart for difficulty: ${Highscore.formatSong('', curDifficulty).replace('-', '')}';
        if (FlxG.random.bool(1 / 100 * (10 * 'undertale'.length))) alertTitle = 'But it refused...';
        FlxG.stage.window.alert(alertTitle, 'Missing chart: ${Paths.chart(songs[curSelected].songName, poop)}');
      }
    }
  }
  ...
  function changeDiff(change:Int = 0)
  {
    ...
    {
      final poop:String = Highscore.formatSong(song.text.toLowerCase(), curDifficulty);
      song.color = Assets.exists(Paths.chart(song.text, poop)) ? FlxColor.WHITE : FlxColor.GRAY;
    }
  }
